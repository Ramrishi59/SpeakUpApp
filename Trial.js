...
function setFooterVisible(on) {
  ...
  function attachIntroAudioOverlayOnce(src, images, onDone) {
    if (!els.introStartAudio) return;
    ...
    const handler = async () => {
      try {
        ...
        startFloatsOnce(images);            // <-- start floats immediately on tap
        ...
        showIntroAudioOverlay(false);       // hide anyway; hard-cap will take over
      } finally {
    ...
    try {
      ...
      if (p && typeof p.then === 'function') {
        p.then(() => {
          ...
          // autoplay blocked — overlay path will handle it
        });
      } else {
        // very old browsers
        audio.onended = finish;
...
async function render(i) {
  ...
  // 3) floating images will start as soon as audio actually begins
  floatsStarted = false;  // reset per intro render
  const introPics = pickIntroImages(unitMeta || {}, 5); // <-- ADD THIS LINE 
  ...
  if (item.audio) {
    ...
    setTimeout(() => {
      if (!audio.paused) {
        ...
        startFloatsOnce(introPics);
      } else {
        // Autoplay blocked → show button and start floats on tap
        showIntroAudioOverlay(true);
        attachIntroAudioOverlayOnce(item.audio, introPics, () => {
          // When audio finishes after the tap, finish the intro
          clearTimeout(hardCapTimer);
          fadeOutIntroAndRevealUI();
        });
      }
